...
RUN set -xe \
    ...
    && (cd "$USER_HOME_DIR"; su node -c "npm install -g @angular/cli@$NG_CLI_VERSION; npm install -g yarn; chmod +x /usr/local/bin/yarn; npm install -g bower gulp-cli node-gyp node-sass --unsafe; npm cache clean --force")
